...
class Youtunee {
  ...
  Future<Map<String, dynamic>> getOrSetContext() async {
    ...
    final response = await client.get(Uri.https('music.youtube.com'), headers: headers);
  ...
  Content parseContent(dynamic item) {
    final tmpRuns = (item['flexColumns'] as List<dynamic>)
      .map((fc) => fc['musicResponsiveListItemFlexColumnRenderer']['text']['runs'])
      .where((r) => r != null)
      .expand((i) => i)
      .toList();
    ...
    final tmpThumbnail = (item['thumbnail']['musicThumbnailRenderer']['thumbnail']['thumbnails'] as List).last['url'] as String;
    ...
    if(item['navigationEndpoint'] != null) {
      ...
      title.type = (browserEndpoint['browseEndpointContextSupportedConfigs']['browseEndpointContextMusicConfig']['pageType'] as String).replaceAll("MUSIC_PAGE_TYPE_", "").toLowerCase();
    }

    return Content(id: title.id ?? '', type: title.type ?? '', thumbnail: thumbnail, title: title, subtitle: subtitle);
  }
  ...
  Future<List<CategorizedContent>?> getFeatured() async {
    ...
    final carouselShelfsData = jsonData['contents']['singleColumnBrowseResultsRenderer']['tabs'][0]['tabRenderer']['content']['sectionListRenderer']['contents'];
    ...
    if(carouselShelfsData.length > 1) {
      for (var i = 0; i < carouselShelfsData.length - 1; i++) {
        ...
        for (var i = 0; i < (musicCarousel['contents'] as List<dynamic>).length; i++) {
          ...
          if(mtrir != null) {
            final title = TextComponentDetail
              .create(
                (mtrir['title']['runs'] as List).where((r) => r != null).toList()
              ).first;
            final subtitle = TextComponentDetail
              .create(
                (mtrir['subtitle']['runs'] as List).where((r) => r != null).toList()
              );
            final tmpThumbnail = (mtrir['thumbnailRenderer']['musicThumbnailRenderer']['thumbnail']['thumbnails'] as List).last['url'] as String;
            final thumbnail = tmpThumbnail.startsWith('https://i.ytimg.com') 
              ? tmpThumbnail
              : tmpThumbnail.replaceFirst(RegExp(r"=w\d+-h\d+"), '=w512-h512');
  ...
  Future<List<CategorizedContent>?> search({ required String query, String? ctoken, String? params }) async {
    ...
    final url = Uri.https('music.youtube.com', '/youtubei/v1/search', queryParams);
    final client = http.Client();
    final response = await client.post(url, headers: headers, body: jsonEncode(requestBody));
    if(response.statusCode != 200) return null;

    final jsonData = jsonDecode(response.body);
    if(jsonData['contents'] != null) {
      final slr = jsonData['contents']['tabbedSearchResultsRenderer']['tabs'][0]['tabRenderer']['content']['sectionListRenderer']['contents'];
      ...
      for (var i = 0; i < (slr as List<dynamic>).length; i++) {
        ...
        result.add(CategorizedContent(category: category, contents: contents, next: next));
      }
      return result;
    }
  ...
  Future<Playlist?> getPlaylistItems(String playlistId) async {
    ...
    final stringData = unescape(rawStringData);
    final jsonData = jsonDecode(stringData);
    final mrhr = jsonData['contents']['twoColumnBrowseResultsRenderer']['tabs'][0]['tabRenderer']['content']['sectionListRenderer']['contents'][0]['musicResponsiveHeaderRenderer'];
    final slrc = jsonData['contents']['twoColumnBrowseResultsRenderer']['secondaryContents']['sectionListRenderer']['contents'][0]['musicShelfRenderer']['contents'] as List<dynamic>;
    ...
    final List<TextComponentDetail> description = mrhr['description'] == null 
      ? [] 
      : TextComponentDetail.create(mrhr['description']['musicDescriptionShelfRenderer']['description']['runs'] ?? []);
    final tmpThumbnail = (mrhr['thumbnail']['musicThumbnailRenderer']['thumbnail']['thumbnails'] as List).last['url'] as String;
    final thumbnail = tmpThumbnail.startsWith('https://i.ytimg.com') 
      ? tmpThumbnail
      : tmpThumbnail.replaceFirst(RegExp(r"=w\d+-h\d+"), '=w512-h512');
    List<Content> contents = [];
    ...
    for (var i = 0; i < slrc.length; i++) {
      if(slrc[i]['musicResponsiveListItemRenderer'] != null) {
        ...
        final tmpRuns2 = (item['fixedColumns'] as List<dynamic>)
          .map((fc) => fc['musicResponsiveListItemFixedColumnRenderer']['text']['runs'])
          .where((r) => r != null)
          .expand((i) => i)
          .toList();
      ...
      if(slrc[i]['musicMultiRowListItemRenderer'] != null) {
        ...
        final description = TextComponentDetail.create(item['description']['runs']);
        final tmpThumbnail = (item['thumbnail']['musicThumbnailRenderer']['thumbnail']['thumbnails'] as List).last['url'] as String;
        final thumbnail = tmpThumbnail.startsWith('https://i.ytimg.com') 
          ? tmpThumbnail
          : tmpThumbnail.replaceFirst(RegExp(r"=w\d+-h\d+"), '=w512-h512');
  ...
  Future<Queue?> getNextQueue({ required String itemId, String playlistId = '' }) async {
    final context = await getOrSetContext();
    ...
    final response = await client.post(
      Uri.https('music.youtube.com', '/youtubei/v1/next', {'prettyPrint' : 'false'}), 
      headers: headers, 
    ...
    final pid = playlistId.isNotEmpty 
      ? playlistId 
      : playlist.last['automixPreviewVideoRenderer']['content']['automixPlaylistVideoRenderer']['navigationEndpoint']['watchPlaylistEndpoint']['playlistId'];
    ...
    if(hasLyrics) {
      lyricsBrowseId = wntrrTabs[1]['tabRenderer']['endpoint']['browseEndpoint']['browseId'];
    }
    ...
    for (var i = 0; i < playlist.length; i++) {
      ...
      final title = runs.first;
      final subtitle = runs.sublist(1);
      final tmpThumbnail = (item['thumbnail']['thumbnails'] as List).last['url'] as String;
      final thumbnail = tmpThumbnail.startsWith('https://i.ytimg.com') 
        ? tmpThumbnail
        : tmpThumbnail.replaceFirst(RegExp(r"=w\d+-h\d+"), '=w512-h512');
